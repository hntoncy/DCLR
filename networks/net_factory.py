...
# from networks.VAE import UncertaintyVAE
from networks.efficientunet import Effi_UNet
from networks.enet import ENet
from networks.my_unet import UNet_f
from networks.pnet import PNet2D
from networks.unet import UNet, UNet_DS, UNet_URPC, UNet_CCT, UNet_f, UNet_2d
import argparse

from networks.unet_CGS import OVRUNet
from networks.unet_bcp import UNet_BCP,UNet_2d
from networks.unet_cmmt import MCNet2d_v1
from networks.unet_decseg import Unet_Decseg
# from networks.VAE import UncertaintyVAE
# from networks.VAE_GAN import VAE, Discriminator
...
"""
模型类型        	      描述	                               适用场景
unet	            标准UNet	                            通用医学图像分割
unet_ds/unet_cct	改进版UNet（Deep Supervision/CCT）	复杂结构分割任务
unet_urpc	        UNet结合不确定性估计	                需要置信度输出的场景
efficient_unet	    基于EfficientNet的UNet变体	        资源受限环境下的高效分割
ViT_Seg	            基于Swin Transformer的架构	        需要长距离上下文建模的任务
pnet	            自定义金字塔网络	                    多尺度特征分析
nnUNet	            经过医学影像优化的UNet变体	            医学影像标准化分割任务
"""
...
parser.add_argument(
    '--cfg', type=str, default="D:/Code/2/Mymodel/code/configs/swin_tiny_patch4_window7_224_lite.yaml", help='path to config file', )
parser.add_argument(
    "--opts",
...
def net_factory(net_type="unet", in_chns=1, class_num=2):
    if net_type == "unet":
        ...
        net = UNet_f(in_chns=in_chns, class_num=class_num).cuda()
    elif net_type == "unet_bcp":
        net = UNet_BCP(in_chns=in_chns, class_num=class_num).cuda()
    elif net_type == "unet_cmmt":
        net = MCNet2d_v1(in_chns=in_chns, class_num=class_num).cuda()
    elif net_type == 'unet_CGS':
        net = OVRUNet(in_chns=in_chns, class_num=class_num).cuda()
    elif net_type == 'unet_decseg':
        net = Unet_Decseg(in_chns=in_chns, class_num=class_num).cuda()
    # elif net_type == "vae":
    #     net = UncertaintyVAE(in_channels=in_chns).cuda()
    # elif net_type == "vae_2":
    #     net = VAE().cuda()
