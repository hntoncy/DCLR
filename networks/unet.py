...
class Decoder_f(nn.Module):
    def __init__(self, params):
        ...
        self.decoder_features = []  # 新增：存储解码器中间特征
...
class UNet_f(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.features = {}  # 新增：存储解码器中间特征

        self.feat_fusion = FeatureFusion(feat_channels=[128, 64, 32, 16])  # up1~up4的通道数

    def forward(self, x):
        feature = self.encoder(x)
        output = self.decoder(feature)
        self.decoder_features = self.decoder.decoder_features  # 存储解码器特征
        self.frs = self.feat_fusion(self.decoder_features)  # 学生模型指导特征
        # 存储中间特征
        self.features['up1'] = self.decoder_features[0]  # VAE特征融合使用 (B, 128, 64,64)
        self.features['up2'] = self.decoder_features[1]  # VAE特征融合使用 (B, 64, 128, 128)
        self.features['up3'] = self.decoder_features[2]  # VAE特征融合使用 (B, 32, 256, 256)
        self.features['up4'] = self.decoder_features[3]  # 图注意力原型对比学习使用 (B, 16, 512, 512)
...
class FeatureFusion(nn.Module):
    def __init__(self, feat_channels, out_channels=64):
        super().__init__()
        # feat_channels: up1~up4的通道数，如[512, 256, 128, 64]
        self.convs = nn.ModuleList([
            nn.Conv2d(in_ch, out_channels, kernel_size=1)  # 1x1卷积统一通道数
            for in_ch in feat_channels
        ])
        self.fuse_conv = nn.Sequential(  # 融合拼接后的特征
    ...
    def forward(self, features):
        """
        features: 列表，包含up1~up4的特征张量
        return: 融合后的指导特征fr (B, out_channels, H, W)
        """
        # 上采样所有特征到最大尺度（up4的尺度，即H/2, W/2）
        target_size = features[-1].shape[2:]  # up4的空间尺寸
        fused = []
        ...
        # 拼接并融合
        fused = torch.cat(fused, dim=1)  # (B, out_channels*4, H, W)
        fr = self.fuse_conv(fused)  # (B, out_channels, H, W)
        return fr
